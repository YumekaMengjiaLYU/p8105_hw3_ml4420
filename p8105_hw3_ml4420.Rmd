...
```
The variables that exist are `r names(accel_data)`.
The dataset has `r nrow(accel_data)` rows and `r ncol(accel_data)` and `r n(accel_data)` many observations.
...
accel_data_plot = accel_data %>%
 ...
 mutate(hour_id = factor(floor(minute/60) + 1)) %>%
 ...
 # scale_x_continuous(breaks = seq(60, 1440, 60), labels = minute_to_hour) +
